...
public class RubberStamp {
    ...
    private void addTextToBitmap(@NonNull RubberStampConfig config,
        ...
        if (config.getRubberStampPosition() != TILE) {
            ...
            int xMargin = config.getXMargin();
            int yMargin = config.getYMargin();
            Bitmap textImage = Bitmap.createBitmap((int)rubberStampMeasuredWidth+xMargin,
                    rubberStampHeight+yMargin,
